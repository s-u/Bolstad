...
binomixp = function(x, n, alpha0 = c(1, 1), alpha1 = c(1, 1), p = 0.5, ...) {
  ...
  if (Bolstad.control(...)$plot) {
    ...
    y.max = max(prior.0[is.finite(prior.0)], prior.1[is.finite(prior.1)], prior[is.finite(prior)])
    
    plot(pi[is.finite(prior.0)], prior.0[is.finite(prior.0)], ylim = c(0, y.max * 1.1), xlab = expression(pi), ylab = "Density", main = "Mixture prior and its components", 
         type = "l", lty = 2, col = "black")
    lines(pi[is.finite(prior.1)], prior.1[is.finite(prior.1)], lty = 3, col = "red")
    lines(pi[is.finite(prior)], prior[is.finite(prior)], lty = 1, col = "green")
    legend("topleft", cex = 0.7, bty = "n", legend = c(expression(prior[0]), expression(prior[1]), expression(prior[mix])), lty = c(2, 
                                                                                                                                    3, 1), col = c("black", "red", "green"))
    
    ## plot the posteriors and the mixture posterior
    
    y.max = max(posterior.0, posterior.1, posterior)
    
    plot(pi, posterior.0, ylim = c(0, y.max * 1.1), xlab = expression(pi), ylab = "Density", main = "Mixture posterior and its components", 
         type = "l", lty = 2, col = "black")
    lines(pi, posterior.1, lty = 3, col = "red")
    lines(pi, posterior, lty = 1, col = "green")
    legend("topleft", bty = "n", legend = c(expression(posterior[0]), expression(posterior[1]), expression(posterior[mix])), lty = c(2, 
                                                                                                                                     3, 1), col = c("black", "red", "green"))
    ...
    lines(pi, posterior, lty = 1, col = "green")
    legend("topleft", bty = "n", legend = c(expression(prior[mix]), expression(likelihood), expression(posterior[mix])), lty = c(2, 
                                                                                                                                 3, 1), col = c("black", "red", "green"))
    
    par(o.par)
  }
  
  results.comp1 = list(name = "pi", param.x = pi, prior = prior.0, likelihood = likelihood, posterior = posterior.0)
  class(results.comp1) = "Bolstad"
  
  results.comp2 = list(name = "pi", param.x = pi, prior = prior.1, likelihood = likelihood, posterior = posterior.1)
  class(results.comp2) = "Bolstad"
  
  results.mix = list(name = "pi", param.x = pi, prior = prior, likelihood = likelihood, posterior = posterior, 
